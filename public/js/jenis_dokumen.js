...
var limit, jenis, jenis_dokumen, request
...
function table() {
  ...
  dTable = $('#datatable').DataTable( {
    ...
    "serverSide": true,
    "ajax": {
      "url": base_url + '/admin-master-jenis-dokumen-data',
      "dataType": "json",
      "data": function ( d ) {
         d.limit = limit;
     },
...
$( "#proses_data" ).click(function() {
  ...
  var data = {
    proses        :$("#proses").val(),
    enc_id        :$("#enc_id").val(),
    nama_jenis_dokumen         :$("#nama_jenis_dokumen").val(),
  }
  $.ajax({
      url: base_url + '/admin-master-jenis-dokumen-proses',
      method: "POST",
      data: data,
...
$(document).on('click', '.openModal', function(){
  ...
  if (id == '') {
    ...
    $("#proses").val('create');
    $("#nama_jenis_dokumen").val('');
    $('#enc_id').val('');
  }else{
    $.ajax({
        url: base_url + '/admin-master-jenis-dokumen-get-id',
        method: "POST",
        data: {id:id},
        success: function(res){
          $('.loading').hide();
          ...
          if (hasil['status'] == 'success') {
            ...
            $("#nama_jenis_dokumen").val(hasil['data'][0]['nama_jenis_dokumen']);
