...
class PejabatController extends Controller
{
  ...
  {
    return view('admin.master.pejabat.index');
  }
  ...
  public function data(Request $request)
  {
    ...
    $get_data = Pejabat::where(function ($query) use ($search) {
                      $query->where('nama_pejabat', 'like', '%'.$search.'%');
                  });
    $recordsFiltered = $get_data->count();
    $recordsTotal = Pejabat::count();
    ...
    if (empty($data)) {
      ...
      foreach ($data as $key => $val) {
        ...
        $data_final[] = '<span>'.$val['nama_pejabat'].'</span>';
  ...
  public function data_id(Request $request)
  {
    ...
    $data = Pejabat::where('id', '=', $id)->get()->toArray();
    $response = [
      'status' => 'success',
      'code' => 200,
      'data' => $data,
    ...
    if ($validator->passes()) {
      if ($params['proses'] == 'create') {
        $data_params = array(
          ...
          'nama_pejabat' => $params['nama_pejabat'] != '' ? $params['nama_pejabat'] : '',
        );

        $data = new Pejabat;

        $data->nama_pejabat = $data_params['nama_pejabat'];
        ...
        try {
          Pejabat::where("id", $id)->update($data_params);
        } catch (\Exception $e) {
          $response = [
          'status' => 'error',
          'pesan' => 'Gagal mengubah pejabat!',
          'error' => $e->getMessage(),
          'code' => 500,
          ];
        }
  ...
  public function kelola(Request $request)
  {
    ...
    if ($action == 'publish') {
      ...
      foreach ($data as $key => $value) {
        $value = str_replace('selected_', '', $value);
        $dec_id = decrypt($value);
        Pejabat::where('id',$dec_id)->delete();
      }
    }
    return redirect()->route('admin_master_pejabat');
  }
}
